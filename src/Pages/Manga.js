import { useMangaStyles } from "../Styles/Manga1Styles";
import { useMobileStyles } from "../Styles/Manga1MobileStyles";
import { useEffect, useState } from "react";
import Button from "@material-ui/core/Button";
import Progress from "@material-ui/core/LinearProgress";
import axios from "axios";
import { tabList } from "../Constants/MangaTabList";
import { Scrollbars } from "react-custom-scrollbars";
import { GiBarbedStar } from "react-icons/gi";
import useMediaQuery from "@material-ui/core/useMediaQuery";

export default function MangaComponent() {
  const classes = useMangaStyles();
  const mobile = useMobileStyles();
  const [isLoading, setLoading] = useState(true);
  const [currentTab, setTab] = useState("manga");
  const [array, setAPIArray] = useState([]);
  const screenWidth = useMediaQuery("(min-width:600px)");

  useEffect(() => {
    const url = `https://api.jikan.moe/v3/top/manga/1/${currentTab}`;
    axios
      .get(url)
      .then((res) => {
